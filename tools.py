...
class AddPetInput(BaseModel):
    ...
    tags: Optional[List[dict]] = Field(
        default=None,
        description="List of tag dictionaries (with 'id' and 'name').")
...
class UpdatePetInput(BaseModel):
    ...
    tags: Optional[List[dict]] = Field(default=None,
                                       description="List of tag dicts.")
    status: str = Field(default="available",
                        description="Pet status (default 'available').")
...
class FindPetsByTagsInput(BaseModel):
    tags: str = Field(...,
                      description='Comma-separated string, e.g. "tag1,tag2"')
